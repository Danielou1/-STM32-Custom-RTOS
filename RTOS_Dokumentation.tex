...
\subsection{Vorschau: Priority Inheritance (Prioritätsvererbung)}
Um das Problem der \textbf{Prioritätsinversion} zu lösen, wird aktuell die Prioritätsvererbung implementiert. 
\begin{enumerate}
    \item \textbf{Szenario:} Eine hochpriore Task wartet auf einen Mutex, den eine niedrigpriore Task hält.
    \item \textbf{Aktion:} Die niedrigpriore Task "erbt" temporär die Priorität der wartenden hochprioren Task.
    \item \textbf{Ziel:} Die niedrigpriore Task kann schneller fertig werden (wird nicht von mittleren Tasks unterbrochen) und gibt die Ressource für die hochpriore Task frei.
\end{enumerate}
Status: Die TCB-Felder und der prioritätsbasierte Scheduler sind bereits vorbereitet.
